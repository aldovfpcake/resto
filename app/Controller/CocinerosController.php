	...
	public function add() {
		if ($this->request->is('post')) {
			...
			if ($this->Cocinero->save($this->request->data)) {
				$this->Session->setFlash(__('The cocinero has been saved.'));
 ...
 */
	...
	public function delete($id = null) {
		...
		if ($this->Cocinero->delete()) {
			$this->Session->setFlash(__('The cocinero has been deleted.'));
		} else {
			$this->Session->setFlash(__('The cocinero could not be deleted. Please, try again.'));
		}
		return $this->redirect(array('action' => 'index'));
	}
}
